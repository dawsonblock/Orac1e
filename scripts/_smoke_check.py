...
# ── 6. preflight ─────────────────────────────────────────────────────────────
try:
    from integration.preflight import load_config
    # Should not raise
    cfg = load_config()
...
try:
    ...
    # Check for requirements install (either direct pip or via VENV_PIP variable)
    assert "pip install -r requirements.txt" in bs or "VENV_PIP" in bs, "requirements install missing"
    assert "pip install -e ." in bs or "VENV_PIP" in bs, "root editable install missing"
    assert "pip install -e third_party/aider" in bs or "VENV_PIP" in bs, "aider editable install missing"
    assert "pip install -e third_party/code-agent-runtime" in bs or "VENV_PIP" in bs, "code-agent-runtime editable install missing"
    assert "pip install -e third_party/cocoindex-code" in bs or "VENV_PIP" in bs, "cocoindex editable install missing"
    # Check for import verification (either "Import checks passed" or "Verifying runtime imports")
    assert "Import checks passed" in bs or "Verifying runtime imports" in bs, "import verification missing"
