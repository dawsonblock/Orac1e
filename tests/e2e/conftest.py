...
@pytest.fixture()
def promotion_env_committed(tmp_path, monkeypatch):
    """Like promotion_env, but worktree has a committed diff so promote_run can capture HEAD~1..HEAD."""
    root = tmp_path / "repo"
    workspace_runs = root / "workspace" / "runs"
    metadata_dir = workspace_runs / "metadata"
    metadata_dir.mkdir(parents=True, exist_ok=True)
    ...
    # Write a fix into the worktree and commit it so HEAD~1..HEAD captures the diff
    ...
    subprocess.run(["git", "-C", str(worktree), "add", "app.py"], check=True)
    subprocess.run(["git", "-C", str(worktree), "commit", "-m", "fix: update app"], check=True, capture_output=True, text=True)
